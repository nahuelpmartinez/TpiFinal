...
while (vendedor.getID() == -1) {
    cout << "No existe un vendedor con código: " << codVendedor << "." << endl;
    cout << "Ingrese un nuevo código de vendedor o 0 para volver al menú de ventas: ";
    cin >> codVendedor;

    system("cls");

    if (codVendedor == 0) {
        cout << "Volviendo al menú de ventas..." << endl;
        system("pause");
        system("cls");
        return MenuVentas(); // Redirigir al menú de ventas
    }
    ...
    for (int i = 0; i < cantidadArticulos; i++) {
        ...
        while (!archivoArticulos.buscarArticuloPorCod(codArticulo, articuloActual)) {
            cout << "El articulo con codigo " << codArticulo << " no existe. Vuelva a ingresarlo: ";
            cin >> codArticulo;
        }
    ...
    if (registroVenta && registroDetalles > 0) {
        cout << "La venta se registro con éxito y el stock fue actualizado." << endl;
    } else {
        cout << "Ocurrio un error al registrar la venta o actualizar los detalles." << endl;
    }
